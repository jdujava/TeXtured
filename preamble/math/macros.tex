...
\RenewDocumentCommand{\dd}{s O{} m}{
    \mathinner{
        \IfBooleanTF{#1}{\mathbf{d}}{\mathrm{d}}^{\mspace{-1mu}#2}#3
    }
}
%% Bold differential
...
\NewDocumentCommand{\dAlambertian}{s}{
    \mathord{
        \.
...
\NewDocumentCommand{\scalemath}{m}{\mathpalette{\scalemathAux{#1}}}% \scalemath{<factor>}{...}
\NewDocumentCommand{\scalemathAux}{mmm}{\scalebox{#1}{\(#2#3\)}}
...
\NewDocumentCommand{\idot}{}{\sbullet[0.4]}
...
\NewDocumentCommand{\longleadsto}{O{}O{}}{%
    \ensuremath{\mathrel{
            \tikz[baseline=-0.5ex, inner sep=0ex, font=\scriptsize]{
                \node[minimum width=2.15em, inner xsep=0.6em, align=center] (a){\hphantom{#1}\\[0ex]\hphantom{#2}};
                \IfBlankF{#1}{\node[inner sep=0.3ex, above=0.3ex, xshift=-0.1em] at (a){#1};}
                \IfBlankF{#2}{\node[inner sep=0.3ex, below=0.3ex, xshift=-0.1em] at (a){#2};}
