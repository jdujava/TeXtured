...
\makeatletter
\if@twoside%
    \def\Gm@bindingoffset{15mm} % binding offset for two-sided printing
\fi%
\makeatother
\geometry{
    width=145mm,
...
%% try to make all pages have the same height (default for `twoside`)
\flushbottom

%% following text appears on a next page (right-hand page for two-sided printing)
%% (empty page is inserted if necessary for two-sided printing)
\NewCommandCopy{\oldcleardoublepage}{\cleardoublepage}
\RenewDocumentCommand{\cleardoublepage}{}{\begingroup\pagestyle{empty}\oldcleardoublepage\endgroup}
